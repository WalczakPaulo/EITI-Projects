...
PRIVATE int whichFit = FIRST_FIT; 
...
PUBLIC int do_hole_map(void)
{
  ...
  phys_clicks buffer[NR_HOLES * 2 + 1], count, i = 0; /*lengths + addresses + 0 */
...
{
  if(mm_in.m1_i1 != FIRST_FIT && mm_in.m1_i1 != WORST_FIT)
	 return -1;
  whichFit = mm_in.m1_i1;
  return 0;
}
 ...
 *===========================================================================*/
  ...
  */
  ...
  register struct hole *hp, *prev_ptr, *max_block, *prev_max_block;
  phys_clicks old_base;
  if(whichFit==FIRST_FIT) {
      do {
       	hp = hole_head;
      ...
      do {
        prev_max_block = NIL_HOLE;	/* prev_max_block - hole previous to max*/
 	      max_block = hp = hole_head;	/* max_block - biggest hole */
 	      while(hp != NIL_HOLE && hp->h_base < swap_base) { /*find biggest hole*/
          if(hp->h_len > max_block->h_len) {
            max_block = hp;
            prev_max_block = prev_ptr;
          }
          prev_ptr = hp;
    	    hp = hp->h_next;
 	      }
       	if(max_block->h_len >= clicks) {
       		/*Use the biggest hole */
       		old_base = max_block->h_base;
       		max_block->h_base += clicks;
       		max_block->h_len -= clicks;

 		/* Delete the hole if used up completely. */
       		if(!max_block->h_len)
       			del_slot(prev_max_block, max_block);
