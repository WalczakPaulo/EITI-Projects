...
**Tips:**
  ...
  * `ElementAre*()` works with _any_ container that implements the STL iterator concept (i.e. it has a `const_iterator` type and supports `begin()/end()`) and supports `howManySticks()`, not just the ones defined in STL. It will even work with container types yet to be written - as long as they follows the above pattern.
